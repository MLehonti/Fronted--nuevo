  ...
  return (
    <nav className={`navbar ${menuOpen ? 'active' : ''}`}>
      ...
      <ul className="navbar-links">
        ...
        {/* Menú Gestión solo para ADMIN y ADMIN_DE_PRODUCTOS */}
        {(roles.includes('ROLE_ADMIN') || roles.includes('ROLE_ADMIN_DE_PRODUCTOS')) && (
          <li
            className={`productos-dropdown ${dropdownOpen['gestion'] ? 'active' : ''}`}
            onClick={() => toggleDropdown('gestion')}
          >
            <Link to="#">Gestión</Link>
            <ul className="dropdown-menu">
              {/* Mostrar la opción de Usuarios solo para ADMIN */}
              {roles.includes('ROLE_ADMIN') && (
                <li><Link to="/usuarios">Usuarios</Link></li>
              )}
              {/* Mostrar la opción de Inventario para ADMIN y ADMIN_DE_PRODUCTOS */}
              <li><Link to="/inventariolist">Inventario</Link></li>
            </ul>
          </li>
        )}
